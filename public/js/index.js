function init() {
  ...
  */
  socket.on('connect', function () {
    ...
    socket.emit('newUser', {id: sessionId, name: $('#name').val(), channel: $('#channel').val()});
  });
  ...
  */
  ...
  /*
   When the server fires the "channelChanged" event, change user list
  */
  socket.on('channelChanged', function (data) {
    updateParticipants(data.participants);
  });
  ...
  */
  function sendMessage() {
    ...
    $.ajax({
      ...
      type: 'POST',
      dataType: 'json',
      data: {message: outgoingMessage, name: name, channel: channel}
    });
  }
  ...
  */
  function nameFocusOut() {
    var name = $('#name').val();
    var channel = $('#channel').val();
    socket.emit('nameChange', {id: sessionId, name: name, channel: channel});
  }

  /*
   When a user changes their channel, let the server know by
   emitting the "channelChange" event
  */
  function channelFocusOut() {
     var channel = $('#channel').val();
     socket.emit('channelChange', {id: sessionId, channel: channel});
  }
  ...
  $('#channel').on('focusout', channelFocusOut);
